...
class MyVisulizer(Visualizer):
    ...
    def display_coverage(self):
        res_dir = osp.join(cfg.result_dir, f"{args.iter}_{action}_coverage_full.pkl")
        print(res_dir)
...
def test_coverage():
    ...
    for k, res in data_res_full.items():
        ...
        print(f"{res['percent']} | {np.mean(res['values']):.3f} | {k}")
        if res["percent"] == 1:
            coverage += 1
        
    print(f"Coverage of {coverage} out of {data_loader.get_len()}")
    res_dir = osp.join(cfg.result_dir, f"{args.iter}_{action}_coverage.pkl")
    res_full_dir = osp.join(cfg.result_dir, f"{args.iter}_{action}_coverage_full.pkl")
...
if __name__ == "__main__":
    ...
    cfg.mujoco_model_file = f"humanoid_smpl_neutral_mesh_{action}.xml"
    args.vis_model_file = f"humanoid_smpl_neutral_mesh_{action}_vis"
