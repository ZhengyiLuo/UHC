...
class AgentUHM(AgentPPO):
    ...
    def setup_policy(self):
        ...
        self.policy_net = policy_net = policy_dict[cfg.policy_name](cfg, data_sample, device=device, dtype=dtype, mode=self.mode)
    ...
    def setup_env(self):
        ...
        self.cc_cfg = cc_cfg = CC_Config(cfg_id=cfg.cc_cfg, base_dir="")
        ...
        self.env = env_dict[self.cfg.env_name](cfg, cc_cfg=cc_cfg, init_context=context_sample, cc_iter=cfg.policy_specs.get('cc_iter', -1), mode="train")
        self.env.seed(cfg.seed)

    def setup_optimizer(self):
        cfg, device, dtype = self.cfg, self.device, self.dtype
        if cfg.policy_specs.get("policy_optimizer", 'Adam'):
            self.optimizer_policy = torch.optim.Adam(self.policy_net.parameters(), lr=cfg.policy_specs['policy_lr'], weight_decay=cfg.policy_specs['policy_weightdecay'])
        else:
            self.optimizer_policy = torch.optim.SGD(self.policy_net.parameters(), lr=cfg.policy_specs['policy_lr'], momentum=cfg.policy_specs['policy_momentum'], weight_decay=cfg.policy_specs['policy_weightdecay'])

        if cfg.policy_specs.get("value_optimizer", 'Adam'):
            self.optimizer_value = torch.optim.Adam(self.value_net.parameters(), lr=cfg.policy_specs['value_lr'], weight_decay=cfg.policy_specs['value_weightdecay'])
        else:
            self.optimizer_value = torch.optim.SGD(self.value_net.parameters(), lr=cfg.policy_specs['value_lr'], momentum=cfg.policy_specs['policy_momentum'], weight_decay=cfg.policy_specs['value_weightdecay'])
    ...
    def log_train(self, info):
        ...
        if not cfg.no_log:
            wandb.log({"rewards": log.avg_c_info, "eps_len": log.avg_episode_len, "avg_rwd": log.avg_c_reward}, step=self.iter)
    ...
    def setup_data_loader(self):
        ...
        self.data_loader = data_loader = DatasetAMASSBatch(cfg, data_mode="train", seed=cfg.seed, multiproess=True)
        self.test_data_loaders.append(DatasetAMASSBatch(cfg, data_mode="test", seed=cfg.seed, multiproess=True))
    ...
    def save_checkpoint(self, i_iter):
        ...
        with to_cpu(policy_net, value_net):
            ...
            model_cp = {'policy_dict': policy_net.state_dict(), 'value_dict': value_net.state_dict(), 'running_state': running_state, 'freq_dict': self.freq_dict}
    ...
    def load_curr(self):
        ...
        self.logger.info(f'loading model from checkpoint: {cp_path_curr} for {self.global_start_fr}')

    def train_init(self):
        cfg, device, dtype = self.cfg, self.device, self.dtype
        self.policy_net.train_full_supervised(cfg, self.data_loader, device=self.device, dtype=self.dtype, scheduled_sampling=0.3, num_epoch=self.cfg.policy_specs.get('warm_update_full', 50), eval_freq=self.cfg.policy_specs.get('warm_update_eval', 50), save_func=self.save_checkpoint)
    ...
    def eval_policy(self, epoch=0, dump=False):
        ...
        for data_loader in data_loaders:
            ...
            with to_cpu(*self.sample_modules):
                ...
                [[data_res_metrics[k].append(v) for k, v in res.items() if k in metric_names] for k, res in data_res_coverage.items()]
                data_res_metrics = {k: np.mean(np.concatenate(v)) if len(v[0]) > 0 else np.mean(v) for k, v in data_res_metrics.items()}
    ...
    def eval_seq(self, take_key, loader):
        ...
        with to_cpu(*self.sample_modules):
            with torch.no_grad():
                ...
                curr_env.load_context(self.policy_net.init_context(context_sample))
                state = curr_env.reset()

                if self.running_state is not None:
                    state = self.running_state(state)
                for t in range(10000):
                    res['gt'].append(curr_env.context_dict['qpos'][self.env.cur_t])
    ...
    def sample_worker(self, pid, queue, min_batch_size):
        ...
        while logger.num_steps < min_batch_size:
            ...
            context_dict = self.policy_net.init_context(context_sample, fix_height=False)
    ...
    def update_policy(self, states, actions, returns, advantages, exps):
        ...
        for _ in pbar:
            ...
            pbar.set_description_str(f"PPO Loss: {surr_loss.cpu().detach().numpy():.3f}| ratio: {ratio.mean().cpu().detach().numpy():.3f}")
