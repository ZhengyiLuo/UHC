...
class Robot:
    ...
    def load_from_skeleton(
        ...
        if self.mesh:
            ...
            self.model_dirs.append(f"/hdd/zen/dev/nv/crossroad/amp/data/assets/mesh/smpl/{uuid.uuid4()}")
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    # betas[0, :] = 5
    # smpl_robot.load_from_skeleton(betas=betas, objs_info=obj_info, obj_pose=obj_pose)
    params = smpl_robot.get_params()

    # new_params2 = params + (np.random.random(params.shape) - 0.5)

    smpl_robot.load_from_skeleton(betas=betas, objs_info=None)
    # smpl_robot.load_from_skeleton(betas=betas, objs_info=None, params = new_params2)
    # model = load_model_from_path(f'out/{model_name}_test.xml')
    # model = load_model_from_path('assets/mujoco/swimmer.xml')

    # L_ankle = smpl_robot.bodies[3]
    # smpl_robot.add_child_to_body(L_ankle)
    # R_ankle = smpl_robot.bodies[7]
    # smpl_robot.add_child_to_body(R_ankle)

    # print(smpl_robot.get_params(get_name=True))

    
    smpl_robot.write_xml(f"../../nv/crossroad/amp/data/assets/mjcf/smpl_humanoid.xml")
    model = load_model_from_path(f"../../nv/crossroad/amp/data/assets/mjcf/smpl_humanoid.xml")
